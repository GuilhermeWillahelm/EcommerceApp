...
namespace EcommerceApp.Client.Services.CartService
{
    public class CartService : ICartService
    {
        ...
        public async Task AddToCart(CartItem cartItem)
        {
            ...
            {
                var cart = await _localStore.GetItemAsync<List<CartItem>>("cart");
                if (cart == null)
                {
        ...
        public async Task GetCartItemsCount()
        {
            if (await _authService.IsUserAuthenticated())
            {
                ...
                await _localStore.SetItemAsync<int>("cartItemsCount", count);
            }
            else
            {
                var cart = await _localStore.GetItemAsync<List<CartItem>>("cart");
                await _localStore.SetItemAsync<int>("cartItemsCount", cart != null ? cart.Count : 0 );
            }
        ...
        public async Task<List<CartProductResponse>> GetCartProducts()
        {
            ...
            {
                var cartItems = await _localStore.GetItemAsync<List<CartItem>>("cart");
                if(cartItems != null)
                {
        ...
        {
            var localCart = await _localStore.GetItemAsync<List<CartItem>>("carts");
            ...
            if (emptyLocalCart)
            {
                await _localStore.RemoveItemAsync("cart");
            }
        }
